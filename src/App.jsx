...
export default function App() {
  ...
  const [ttsProvider, setTtsProvider] = useState(() => {
    ...
    return (saved && saved !== 'localclone' && saved !== 'omnivoice') ? saved : 'voicefree';
  });
  ...
  useEffect(() => {
    const buildBootCredentialOverrides = (data = {}) => ({
      ...
      voicefreeModelId: localStorage.getItem('voicefree_model_id') || voicefreeModelId || 'eleven_v3',
  ...
  const buildTelegramCredentials = (overrides = {}) => sanitizeTelegramCredentials({
    ...
    voicefreeModelId: voicefreeModelId || 'eleven_v3',
  ...
  const buildCloudAppSettings = (credentialOverrides = {}) => {
    const fromOverrides = (key, fallback) => (
      Object.prototype.hasOwnProperty.call(credentialOverrides, key) && credentialOverrides[key] !== undefined
        ? credentialOverrides[key]
        : fallback
    );
    ...
    return {
      ...
      imageGlowOpacity,
      ttsProvider,
      selectedVoiceId: fromOverrides('selectedVoiceId', fromOverrides('elevenLabsVoiceId', selectedVoiceId)),
      vclipVoiceId: fromOverrides('vclipVoiceId', vclipVoiceId),
      vclipSpeed: fromOverrides('vclipSpeed', vclipSpeed),
      lucyLabVoiceId: fromOverrides('lucyLabVoiceId', lucyLabVoiceId),
      lucyLabSpeed: fromOverrides('lucyLabSpeed', lucyLabSpeed),
      voicefreeVoiceId: fromOverrides('voicefreeVoiceId', voicefreeVoiceId),
      voicefreeProvider: fromOverrides('voicefreeProvider', voicefreeProvider || 'elevenlabs'),
      voicefreeModelId: fromOverrides('voicefreeModelId', voicefreeModelId || 'eleven_v3'),
      voicefreeSpeed: fromOverrides('voicefreeSpeed', voicefreeSpeed || 1.0),
      socialAccounts,
      activeSocialAccountIds,
      selectedSocialAccountIds,
      fbConnected,
